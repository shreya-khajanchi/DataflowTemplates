 ...
 * SpannerAccessor#writeAtLeastOnce}. Supported operations are {@link Constants#MUTATION_INSERT} and
 * {@link Constants#MUTATION_UPDATE} - both are translated to {@link
 * Mutation#newInsertOrUpdateBuilder} so that transient retries under {@code writeAtLeastOnce} are
 * idempotent - and {@link Constants#MUTATION_DELETE} which uses {@link Mutation#delete} with a key
 * constructed from {@link DataGeneratorTable#primaryKeys}.
 *
 * <p>The Spanner connection details are loaded from a JSON configuration file with the shape
 * {@code {"projectId": ..., "instanceId": ..., "databaseId": ...}} - the same format accepted by
 * {@link SpannerSchemaFetcher}.
 */
public class SpannerDataWriter implements DataWriter {
  ...
  private SpannerConfig loadSpannerConfig() {
    if (sinkConfigPath == null || sinkConfigPath.isEmpty()) {
      throw new IllegalArgumentException(
          "Spanner sink requires a valid configuration file path.");
    }
    try {
  ...
  @VisibleForTesting
  Mutation rowToMutation(DataGeneratorTable table, Row row, String operation) {
    ...
    if (operation != null
        && !Constants.MUTATION_INSERT.equalsIgnoreCase(operation)
        && !Constants.MUTATION_UPDATE.equalsIgnoreCase(operation)) {
      throw new IllegalArgumentException("Unsupported Spanner operation: " + operation);
    }
  ...
  private static Date toSpannerDate(Object value) {
    ...
    if (value instanceof java.sql.Date) {
      return Date.fromJavaUtilDate((java.sql.Date) value);
    }
    if (value instanceof java.util.Date) {
      return Date.fromJavaUtilDate((java.util.Date) value);
    }
    return Date.parseDate(value.toString());
  }
  ...
  private static Timestamp toSpannerTimestamp(Object value) {
    ...
    if (value instanceof java.util.Date) {
      return Timestamp.of((java.util.Date) value);
    }
    return Timestamp.parseTimestamp(value.toString());
  }
