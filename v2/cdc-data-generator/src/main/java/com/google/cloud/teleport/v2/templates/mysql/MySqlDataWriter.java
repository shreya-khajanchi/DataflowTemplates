...
public class MySqlDataWriter implements DataWriter {
  ...
  Shard resolveShard(String shardId) {
    ...
    if (shardId == null || shardId.isEmpty()) {
      // No shard specified - fall back to the first shard (iteration order is stable
      // because loadShards() returns an ordered list).
   ...
   * order). For tables that only consist of primary-key columns the {@code ON DUPLICATE KEY
   * UPDATE} clause degrades to a self-assignment on the first PK column, which is still a valid
   * no-op upsert.
   */
  @VisibleForTesting
