...
public class MySqlDataWriterTest {
  ...
  @Test
  public void testBuildUpsertSql_pkOnlyTableUsesSelfAssignment() {
    // A table whose only writable column is the primary key has no non-PK columns to SET. The
    // upsert statement must still be valid, so we fall back to a self-assignment on the first
    // writable column, which is an idempotent no-op.
    DataGeneratorColumn onlyPk =
        DataGeneratorColumn.builder()
            .name("id")
            .logicalType(LogicalType.INT64)
            .isNullable(false)
            ...
            .scale(null)
            .build();
    DataGeneratorTable table = simpleTable().toBuilder().columns(ImmutableList.of(onlyPk)).build();
    String sql = MySqlDataWriter.buildUpsertSql(table);
    assertThat(sql)
        .isEqualTo("INSERT INTO `users` (`id`) VALUES (?) ON DUPLICATE KEY UPDATE `id` = `id`");
  }
  ...
  public void testWrite_routesToNamedShard() throws Exception {
    MySqlDataWriter w = writer();
    w.write(
        ImmutableList.of(simpleRow(1L, "a")), simpleTable(), "shardB", Constants.MUTATION_INSERT);
  ...
  public void testWrite_emptyRowsIsNoOp() throws Exception {
    MySqlDataWriter w = writer();
    w.write(ImmutableList.of(), simpleTable(), "shardA", Constants.MUTATION_INSERT);
  ...
  public void testWrite_nullRowsIsNoOp() throws Exception {
    MySqlDataWriter w = writer();
    w.write(null, simpleTable(), "shardA", Constants.MUTATION_INSERT);
    verifyNoInteractions(mockConnection, mockStatement);
  }
  ...
  public void testWrite_updateUsesSameUpsertSqlAsInsert() throws Exception {
    ...
    insertWriter.write(
        ImmutableList.of(simpleRow(1L, "a")), simpleTable(), "shardA", Constants.MUTATION_INSERT);
    ...
    updateWriter.write(
        ImmutableList.of(simpleRow(1L, "a")), simpleTable(), "shardA", Constants.MUTATION_UPDATE);
  ...
  public void testWrite_deleteUsesDeleteSql() throws Exception {
    MySqlDataWriter w = writer();
    w.write(
        ImmutableList.of(simpleRow(1L, "a")), simpleTable(), "shardA", Constants.MUTATION_DELETE);
  ...
  public void testWrite_unsupportedOperationThrows() {
    ...
    assertThrows(
        IllegalArgumentException.class,
        () -> w.write(ImmutableList.of(simpleRow(1L, "a")), simpleTable(), "shardA", "MERGE"));
  }
  ...
  // -------------------- Parameter binding (per LogicalType) --------------------
  ...
  public void testSetParameter_nullSetsTypedNull_allTypes() throws Exception {
    int[] expectedJdbc = {
      Types.VARCHAR, Types.BIGINT, Types.DOUBLE, Types.BOOLEAN, Types.VARBINARY, Types.DATE,
      Types.TIMESTAMP, Types.NUMERIC, Types.VARCHAR
    };
  ...
  @Test
  public void testClose_isNoOpAndSafe() {
    MySqlDataWriter w = writer();
    w.close();
    w.close(); // idempotent
  }
  ...
  public void testWrite_bigBatchAddsAllRows() throws Exception {
    ...
    List<Row> rows =
        ImmutableList.<Row>builder()
            .addAll(Collections.nCopies(5, simpleRow(1L, "a")))
            .build();
    w.write(rows, simpleTable(), "shardA", Constants.MUTATION_INSERT);
