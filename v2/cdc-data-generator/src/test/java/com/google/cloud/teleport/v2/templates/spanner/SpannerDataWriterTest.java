...
public class SpannerDataWriterTest {
  ...
  @Test
  public void testWrite_emptyRowsIsNoOp() {
    SpannerDataWriter w = writer();
    w.write(ImmutableList.of(), simpleTable(), "", Constants.MUTATION_INSERT);
    verify(mockDatabaseClient, never()).writeAtLeastOnce(any());
  }

  @Test
  public void testWrite_nullRowsIsNoOp() {
    SpannerDataWriter w = writer();
    w.write(null, simpleTable(), "", Constants.MUTATION_INSERT);
    verify(mockDatabaseClient, never()).writeAtLeastOnce(any());
  }
  ...
  public void testWrite_nullTableThrows() {
    ...
    assertThrows(
        ...
        () -> w.write(ImmutableList.of(simpleRow(1L, "a")), null, "", Constants.MUTATION_INSERT));
  }
  ...
  public void testWrite_unsupportedOperationThrows() {
    ...
    assertThrows(
        IllegalArgumentException.class,
        () -> w.write(ImmutableList.of(simpleRow(1L, "a")), simpleTable(), "", "MERGE"));
  }

  @Test
  public void testWrite_insertBuildsInsertOrUpdateMutation() {
    SpannerDataWriter w = writer();
    w.write(
  ...
  @Test
  public void testWrite_updateOperationBuildsInsertOrUpdateMutation() {
    SpannerDataWriter w = writer();
    w.write(ImmutableList.of(simpleRow(1L, "a")), simpleTable(), "", Constants.MUTATION_UPDATE);
    @SuppressWarnings("unchecked")
    ArgumentCaptor<Iterable<Mutation>> muts = ArgumentCaptor.forClass(Iterable.class);
    verify(mockDatabaseClient).writeAtLeastOnce(muts.capture());
    Mutation m = ImmutableList.copyOf(muts.getValue()).get(0);
    // UPDATE is also translated to INSERT_OR_UPDATE so transient retries don't fail when the row
    // no longer exists (or has already been applied).
    assertThat(m.getOperation()).isEqualTo(Mutation.Op.INSERT_OR_UPDATE);
  }

  @Test
  public void testWrite_deleteOperationBuildsDeleteMutation() {
    SpannerDataWriter w = writer();
    w.write(ImmutableList.of(simpleRow(1L, "a")), simpleTable(), "", Constants.MUTATION_DELETE);
    @SuppressWarnings("unchecked")
    ArgumentCaptor<Iterable<Mutation>> muts = ArgumentCaptor.forClass(Iterable.class);
    verify(mockDatabaseClient).writeAtLeastOnce(muts.capture());
    Mutation m = ImmutableList.copyOf(muts.getValue()).get(0);
  ...
  @Test
  public void testWrite_runtimeExceptionPropagates() {
    when(mockDatabaseClient.writeAtLeastOnce(any()))
        .thenThrow(new RuntimeException("spanner exploded"));
    SpannerDataWriter w = writer();
    RuntimeException ex =
        assertThrows(
            ...
            () ->
                w.write(
                    ...
                    "",
                    Constants.MUTATION_INSERT));
    assertThat(ex).hasMessageThat().contains("spanner exploded");
  }

  @Test
  public void testWrite_deleteMissingPrimaryKeyThrows() {
    Schema schema =
        Schema.builder()
            .addNullableField("id", Schema.FieldType.INT64)
            .addNullableField("name", Schema.FieldType.STRING)
    ...
    assertThrows(
        ...
        () -> w.write(ImmutableList.of(row), simpleTable(), "", Constants.MUTATION_DELETE));
  }

  @Test
  public void testWrite_deleteNoPrimaryKeyTableThrows() {
    DataGeneratorTable noPk = simpleTable().toBuilder().primaryKeys(ImmutableList.of()).build();
    SpannerDataWriter w = writer();
    assertThrows(
        IllegalStateException.class,
        () -> w.write(ImmutableList.of(simpleRow(1L, "a")), noPk, "", Constants.MUTATION_DELETE));
  }

  // -------------------- rowToMutation & per-type conversion --------------------
  ...
  public void testRowToMutation_dateValue() {
    ...
    Mutation m = writer().rowToMutation(singleColTable(c), row, Constants.MUTATION_INSERT);
  ...
  public void testRowToMutation_nullValuesEmitTypedNulls() {
    ...
    Mutation m = writer().rowToMutation(simpleTable(), row, Constants.MUTATION_UPDATE);
  ...
  public void testRowToMutation_missingFieldBindsTypedNull() {
    ...
    Mutation m = writer().rowToMutation(simpleTable(), row, Constants.MUTATION_INSERT);
  ...
  public void testRowToMutation_skipsGeneratedAndSkippedColumns() {
    ...
    Mutation m = writer().rowToMutation(t, row, Constants.MUTATION_INSERT);
  ...
  public void testRowToMutation_deleteKeyWithAllTypes() {
    ...
    for (LogicalType type : LogicalType.values()) {
      ...
      Mutation m = writer().rowToMutation(singleColTable(c), row, Constants.MUTATION_DELETE);
      assertThat(m.getOperation()).isEqualTo(Mutation.Op.DELETE);
  ...
  public void testLoadSpannerConfig_readsJsonFile() throws Exception {
    ...
    w.write(ImmutableList.of(simpleRow(1L, "a")), simpleTable(), "", Constants.MUTATION_INSERT);
  ...
  @Test
  public void testClose_closesAccessor() {
    SpannerDataWriter w = writer();
    w.ensureInitialized();
    w.close();
  ...
  @Test
  public void testPublicConstructor_defaultsToGetOrCreate() {
    // Just make sure the String constructor wires up; don't actually init.
    SpannerDataWriter w = new SpannerDataWriter("unused");
    w.close();
  }

  @Test
  public void testEnsureInitialized_idempotent() {
    SpannerDataWriter w = writer();
    w.ensureInitialized();
    w.ensureInitialized();
    // factory should be used at most once - we just verify no explosion.
  }
  ...
  public void testFactoryReturningNullCausesFailureOnWrite() {
    ...
    assertThrows(
        ...
        () ->
            w.write(
                ImmutableList.of(simpleRow(1L, "a")), simpleTable(), "", Constants.MUTATION_INSERT));
  }

  @Test
  public void testWrite_handlesNullOperationAsUpsert() {
    SpannerDataWriter w = writer();
    w.write(ImmutableList.of(simpleRow(1L, "a")), simpleTable(), "", null);
    @SuppressWarnings("unchecked")
    ArgumentCaptor<Iterable<Mutation>> muts = ArgumentCaptor.forClass(Iterable.class);
    verify(mockDatabaseClient).writeAtLeastOnce(muts.capture());
    assertThat(ImmutableList.copyOf(muts.getValue()).get(0).getOperation())
        .isEqualTo(Mutation.Op.INSERT_OR_UPDATE);
  }
}
