...
public class ConnectionHelperRequestTest {
  ...
  public void testGetters_returnConstructorArguments() {
    List<Shard> shards =
        ImmutableList.of(new Shard("id", "host", "3306", "user", "pass", "db", null, null, null));
    ConnectionHelperRequest request =
        new ConnectionHelperRequest(shards, "key=val", 7, "com.mysql.cj.jdbc.Driver", "SELECT 1");
    ...
    assertThat(request.getDriver()).isEqualTo("com.mysql.cj.jdbc.Driver");
    assertThat(request.getConnectionInitQuery()).isEqualTo("SELECT 1");
    assertThat(request.getJdbcUrlPrefix()).isNull();
  }

  @Test
  public void testGetters_allowEmptyOptionalFields() {
    List<Shard> shards = ImmutableList.of();
    ConnectionHelperRequest request = new ConnectionHelperRequest(shards, "", 1, "driver", "");

    assertThat(request.getShards()).isEmpty();
    assertThat(request.getProperties()).isEmpty();
    assertThat(request.getConnectionInitQuery()).isEmpty();
    assertThat(request.getJdbcUrlPrefix()).isNull();
  }
